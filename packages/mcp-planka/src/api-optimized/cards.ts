...
export async function filterCards(
  ...
  if (options.createdByUserId) params.append('createdByUserId', options.createdByUserId);
  if (options.assignedToUserId) params.append('assignedToUserId', options.assignedToUserId);
  ...
  const endpoint = `/cards/filter${queryString ? `?${queryString}` : ''}`;
