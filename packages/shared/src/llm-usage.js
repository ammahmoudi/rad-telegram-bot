 ...
 */
export async function saveLlmUsage(usage, context, model, finishReason, hasToolCalls, toolCallCount, requestDurationMs) {
    try {
 ...
 */
export async function getUserUsageStats(telegramUserId) {
    const prisma = getPrisma();
 ...
 */
export async function getSessionUsageStats(sessionId) {
    const prisma = getPrisma();
    const stats = await prisma.llmCall.aggregate({
        where: { sessionId },
        _sum: {
